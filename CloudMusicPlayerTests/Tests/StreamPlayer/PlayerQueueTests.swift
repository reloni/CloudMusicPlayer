...
class PlayerQueueTests: XCTestCase {
	...
	func testInitWithNewItems() {
		...
		var currentItem: PlayerQueueItem? = PlayerQueueItem(queue: queue, streamIdentifier: audioItems[0])
		queue.queueEvents.bindNext { result in
	...
	func testToPrevious() {
		...
		queue.current = PlayerQueueItem(queue: queue, streamIdentifier: audioItems[1])
	...
	func testToNextOnLastItemSetCurrentToNil() {
		...
		queue.current = PlayerQueueItem(queue: queue, streamIdentifier: audioItems.last!)
	...
	func testRemoveItem() {
		let queue = PlayerQueue(items: audioItems)
		let itemToRemove = PlayerQueueItem(queue: queue, streamIdentifier: audioItems[1])
	...
	func testNotRemoveItemThatNotExistsInQueue() {
		let queue = PlayerQueue(items: audioItems)
		let notExisted = PlayerQueueItem(queue: queue, streamIdentifier: "fake five")
