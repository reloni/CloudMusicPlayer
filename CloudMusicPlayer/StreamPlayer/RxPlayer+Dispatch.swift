...
extension Observable where Element : PlayerEventType {
	public func dispatch(saveCachedData: Bool = false, httpUtilities: HttpUtilitiesProtocol = HttpUtilities.instance,
	                     fileStorage: LocalStorageProtocol = LocalStorage()) -> Observable<DispatchResult> {
		return self.filter { e in if case .PreparingToPlay = e as! PlayerEvents { return true } else { return false } }
			.flatMap { [unowned self] e -> Observable<DispatchResult> in
				...
				return Observable<DispatchResult>.create { observer in
					...
					let disp = createObservable(createUrlTask, saveData: saveData).shareReplay(1)
						.stream(targetContentType ?? item.streamIdentifier.streamResourceContentType).bindNext { e in
							observer.onNext(e)
							observer.onCompleted()
					}
					
					return AnonymousDisposable {
						print("some dispoding")
						disp.dispose()
					}
				}
		}
